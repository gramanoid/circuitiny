...
- `canonical change type`: one of `added`, `modified`, `removed`, `renamed`, or `metadata-only`.
- Compound changes SHALL report the most significant canonical change type using priority order `modified` > `renamed` > `metadata-only` > `added`/`removed`; the change report MUST include auxiliary secondary flags such as `wasRenamed`, `metadataChanged`, and a human-readable `details` field when multiple terms apply.
- `added`: the source index now lists an asset that was not present in the prior cached index.
- `modified`: the asset content, checksum, format, or model file changed.
- `removed`: the upstream source no longer lists the asset, while local installed bundles remain preserved.
- `renamed`: the upstream identifier or display name changed while a stable source URL/checksum links it to the previous asset.
- `metadata-only`: license, attribution, description, tags, or beginner notes changed without model content changes.
...
- **THEN** the system SHALL preserve installed parts, avoid duplicate catalog IDs, and produce a structured change report in the library dashboard that:
  - shows changed or missing upstream assets without deleting local bundles;
  - includes asset ID, canonical change type, timestamp, source URL, and link to the local bundle when present; and
  - logs a structured source-index event for automation and may surface a non-blocking UI notification.
