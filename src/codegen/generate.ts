...
import type { PhysicalLayout } from '../physical/breadboard'
import { assertNoDraftCatalogParts } from './trust'
...
export function generate(project: Project, options: { physicalMode?: boolean; physicalLayout?: PhysicalLayout } = {}): { files: GeneratedFiles; ir: Ir } {
  assertNoDraftCatalogParts(project)
  const ir = buildIr(project, options)
  const sourceProject = ir.connectivityProject
  ...
  const pinResolver = buildPinResolver(sourceProject)
  ...
  for (const beh of sourceProject.behaviors) {
  ...
  const boardDef = catalog.getBoard(sourceProject.board)
